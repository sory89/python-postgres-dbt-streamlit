...
Structure :
    Schema      → raw
    employees   → 10 employés néerlandais
    products    → 3 produits (stroopwafels)
    promotions  → 3 promotions avec dates et remises
...
def generate_promotions(products: list[tuple]) -> list[tuple]:
    """1 promotion par produit, fenêtre 5–10 jours, sans chevauchement."""
    adjectives   = ["Super Surprise", "Taster's Choice", "Weekend Special",
                    "Flash Deal", "Happy Hour", "Loyalty Bonus"]
    descriptions = [
        "Third customer in line discount!",
        "Two stroopwafels for the price of one!",
        "Weekend flash sale — limited time only!",
        "Loyal customer appreciation offer.",
        "Happy hour special — come early!",
        "Buy two, get one at half price.",
    ]
    rows        = []
    used_ranges = []

    for promo_id, product in enumerate(products, start=1):
        for _ in range(100):
            length    = random.randint(5, 10)
            max_start = (END_DATE - datetime.timedelta(days=length)).toordinal()
            start_ord = random.randint(START_DATE.toordinal(), max_start)
            s         = datetime.date.fromordinal(start_ord)
            e         = s + datetime.timedelta(days=length)
            if not any(not (e < ps or s > pe) for ps, pe in used_ranges):
        ...
        rows.append((
            promo_id,
            product[0],
            f"{product[1]} {random.choice(adjectives)}",
...
def seed():
    print("Génération des données avec Faker…")
